...
function drawRealisticComposite(background, products, globalPosition, gpsValues) {
  ...
  const zones = {
    ...
    front: { x: W * 0.75, y: H * 0.77, spread: W * 0.70 },
  };
  ...
  const generate = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Server mengembalikan respon non-JSON (${response.status}). Jika menggunakan Vercel Free, pastikan total foto tidak melebihi 4.5MB.`);
      }
